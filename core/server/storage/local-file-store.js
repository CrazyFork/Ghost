...
LocalFileStore.prototype.save = function (image, targetDir) {
    ...
    return this.getUniqueFileName(this, image, targetDir).then(function (filename) {
        ...
        ).replace(new RegExp('\\' + path.sep, 'g'), '/'); // unify windows paths that generated by path.relative
...
LocalFileStore.prototype.serve = function (options) {
    ...
    if (options.isTheme) {
        return function downloadTheme(req, res, next) {//:bm - this section of codes :) is so good
            var themeName = options.name,
                ...
                zipBasePath = path.join(os.tmpdir(), utils.uid(10)),//:bm
            ...
            return serveStatic(config.getContentPath('images'), {maxAge: utils.ONE_YEAR_MS, fallthrough: false})(req, res, function (err) {
                if (err) {
                    if (err.statusCode === 404) {
                        return next(new errors.NotFoundError({message: i18n.t('errors.errors.pageNotFound')}));//:todo - where is this error got handled?
                    }
